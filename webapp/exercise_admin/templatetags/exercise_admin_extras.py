...
def test_tab(test_obj, stages_list, instance_files, exercise_files, jstemplate=False):
    ...
    return {
        "test": TemplateTest(),
        "stages": [(TemplateStage(), [(TemplateCommand(), [])])],
        "instance_files": [],
        "exercise_files": [],
    }
...
def has_instance_file(test, instance_file):
    if instance_file.include_file.id in test.required_instance_files.all().values_list(
        "id", flat=True
    ):
        return True
    return False
            ...
            if create:
                ...
                for lang_code, _ in lang_list:
                    setattr(
                        ...
                        "SAMPLE_DEFAULT_NAME_{}".format(lang_code),
                    )
                    setattr(
                        self,
                        "description_{}".format(lang_code),
...
def lang_reminder(lang_code):
    s = '<span class="language-code-reminder" title="The currently selected translation">{}</span>'.format(
        lang_code
    )
    return mark_safe(s)
