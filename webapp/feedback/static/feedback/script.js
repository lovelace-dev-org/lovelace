...
function show_feedbacks (header_elem, expand_str, collapse_str, expand_img, collapse_img) {
    const header = $(header_elem)
    const span = header.children("span")
    const img = header.children("img")
    console.log(header)
    const feedbacks = header.next()
...
function select_stars (star_elem, star_num) {
    const stars = $(star_elem).parent().children("label")
...
function add_feedback_form_callbacks () {
    const feedback_forms = $("form.feedback-form")
    feedback_forms.each(function () {
        ...
        form.submit(function (event) {
            ...
            const result_div = form_parent.children("div.feedback-received")
            result_div.css("display", "none")
            const error_div = form_parent.children("div.feedback-error")
