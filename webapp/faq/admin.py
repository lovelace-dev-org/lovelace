...
class FaqAdmin(TranslationAdmin, VersionAdmin):
    ...
    def get_queryset(self, request):
        ...
        edited = (
            Version.objects.get_for_model(FaqQuestion)
            .filter(revision__user=request.user)
            .values_list("object_id", flat=True)
        )

        content_access = CourseContentAdmin.content_access_list(request, ContentPage).values_list(
            "id", flat=True
        )

        return qs.filter(Q(id__in=list(edited)) | Q(exercise__id__in=list(content_access)))
