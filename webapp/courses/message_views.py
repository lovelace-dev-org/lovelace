...
def process_message_form(request, course, instance, recipients, form_label="", use_bcc=False):
    saved_msgs = SavedMessage.objects.filter(
        course=course
    )
    load_url = reverse("courses:load_message", kwargs={
        "course": course,
        "instance": instance,
        "msgid": 0
    })
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            loaded_message = saved_msgs.filter(
                id=form.cleaned_data["saved_msgs"]
            ).first()
