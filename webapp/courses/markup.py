...
class CodeMarkup(Markup):
    ...
    regexp = re.compile(r"^[{]{3}(highlight=(?P<highlight>[^\s]*))?\s*$")  # TODO: Better settings
    markup_class = ""
    example = (
        "{{{highlight=python3\n"
...
class EmbeddedFileMarkup(Markup):
    ...
    regexp = re.compile(
        r"^\<\!file\=(?P<file_slug>[^\|>]+)(\|link_only\=(?P<link_only>[^>|]+))?\>\s*$"
    )
    markup_class = "embedded item"
...
class EmbeddedPageMarkup(Markup):
    ...
    regexp = re.compile(r"^\<\!page\=(?P<page_slug>[^|>]+)\>\s*$")
    markup_class = "embedded item"
...
class HeadingMarkup(Markup):
    ...
    shortname = "heading"
    description = ""
    regexp = re.compile(r"^\s*(?P<level>\={1,6})\=*\s*.+\s*(?P=level)\s*$")
    markup_class = ""
    example = (
        "= 1st Level Heading =\n"
        "== 2nd Level Heading ==\n"
...
class ListMarkup(Markup):
    ...
    regexp = re.compile(r"^(?P<list_level>[*#]+)(?P<text>.+)$")
    markup_class = ""
    example = (
    ...
    def block(cls, block, settings, state):
        if not state["storage"]:
            state["storage"] = {"block": "list", "value": []}

        for line in block:
            state_level = len(state["storage"]["value"])
            ...
            if new_level > state_level:
                for __ in range(new_level - state_level):
                    state["storage"]["value"].append(tag)
                    yield f"<{tag}>"
            elif new_level < state_level:
                for __ in range(state_level - new_level):
                    top_tag = state["storage"]["value"].pop()
                    yield f"</{top_tag}>"

            if len(state["storage"]["value"]) == new_level:
                if state["storage"]["value"][-1] != tag:
                    top_tag = state["storage"]["value"].pop()
                    yield f"</{top_tag}>"
    ...
    def cleanup(cls, new_block, stored_value, state):
        if new_block != cls.shortname:
            undents = reversed(stored_value)
            state["storage"] = {}
            for undent_lvl in undents:
                yield f"</{undent_lvl}>"
...
class SvgMarkup(Markup):
    ...
    regexp = re.compile(
        r"^[{]{3}svg\|width=(?P<svg_width>[0-9]+)\|height=(?P<svg_height>[0-9]+)\s*$"
    )
    markup_class = ""
    example = ""
    inline = False
...
class TableMarkup(Markup):
    ...
    def block(cls, block, settings, state):
        if not state["storage"]:
            ...
            state["storage"] = {"block": "table", "value": True}
    ...
    @classmethod
    def cleanup(cls, new_block, stored_value, state):
        if new_block != cls.shortname and state["storage"]["value"]:
            state["storage"] = {}
            yield "</table>"
...
def register_markups():
    MarkupParser.register_markup(ParagraphMarkup)
    MarkupParser.register_markup(BoldMarkup)
    MarkupParser.register_markup(CalendarMarkup)
    MarkupParser.register_markup(EmbeddedFileMarkup)
    MarkupParser.register_markup(EmbeddedPageMarkup)
    MarkupParser.register_markup(EmbeddedScriptMarkup)
    MarkupParser.register_markup(EmbeddedVideoMarkup)
    MarkupParser.register_markup(EmptyMarkup)
    MarkupParser.register_markup(HeadingMarkup)
    MarkupParser.register_markup(ImageMarkup)
    MarkupParser.register_markup(ListMarkup)
    MarkupParser.register_markup(SeparatorMarkup)
    MarkupParser.register_markup(SvgMarkup)
    MarkupParser.register_markup(TableMarkup)
    MarkupParser.register_markup(TeXMarkup)
    MarkupParser.register_markup(CodeMarkup)

    LinkParser.register_markup(EmbeddedFileMarkup)
    LinkParser.register_markup(EmbeddedPageMarkup)
    LinkParser.register_markup(EmbeddedScriptMarkup)
    LinkParser.register_markup(EmbeddedVideoMarkup)
    LinkParser.register_markup(ImageMarkup)
    LinkParser.register_markup(ParagraphMarkup)
