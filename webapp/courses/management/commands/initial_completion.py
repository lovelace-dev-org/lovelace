...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        for instance in CourseInstance.objects.all():
            ...
            instance_tasks = EmbeddedLink.objects.filter(instance=instance).order_by(
                "embedded_page__name"
            )
            ...
            for user in users:
                ...
                for __, result in user_results.items():
                    try:
                        completion = UserTaskCompletion.objects.get(
                            exercise=result["eo"], instance=instance, user=user
                        )
    ...
    def _get_results(self, user, task_links, instance):
        ...
        for link in task_links:
            ...
            if link.revision is not None:
                exercise_obj = (
                    Version.objects.get_for_object(exercise_obj)
            ...
            if exercise_obj.content_type == "ROUTINE_EXERCISE":
                try:
                    progress = RoutineExerciseProgress.objects.get(
                        user=user, exercise=exercise_obj, instance=instance
                    )
    ...
    def _get_group(self, task_links, task):
        ...
        ).exclude(embedded_page__id=task.id)
