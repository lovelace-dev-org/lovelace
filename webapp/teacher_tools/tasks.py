    ...
    with io.StringIO() as temp_csv:
        ...
        for i, user in enumerate(users, start=1):
            ...
            results_url = reverse(
                ...
                kwargs={"course": course, "instance": instance, "user": user},
            )
            writer.writerow(
                [
                    user.username,
        ...
        try:
            ...
            try:
                ...
                return {
                    "current": i,
                    "total": users_n,
                    "aborted": _("Sending failed. Try again to resume."),
                }
...
def order_moss_report(self, course_slug, instance_slug, exercise_slug, submit_form):
    ...
    for islug in instances:
        answerers = (
            UserFileUploadExerciseAnswer.objects.filter(
                exercise__slug=exercise_slug, instance__slug=islug
            )
            .distinct("user")
    ...
    for islug, answerers in answerers_by_instance:
        ...
        for i, student in enumerate(answerers):
            ...
            answers_path = os.path.join(
                settings.PRIVATE_STORAGE_FS_PATH, "returnables", islug, student, exercise_slug
            )
            crawl(answers_path)
    ...
    command = [
        ...
        "-l",
        submit_form["language"],
        "-d",
        "-m",
        str(submit_form["matches"]),
    ]
    ...
    self.update_state(
        state="PROGRESS", meta={"phase": "MOSS_SUBMIT", "current": i + 1, "total": users_n}
    )
