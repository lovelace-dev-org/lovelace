...
class Node:
    ...
    def __unicode__(self):
        return u'"%s" [%s]' % (self.data["name"], ",".join(['%s="%s"' % (k, v) for k, v in self.data.iteritems() if k != "name"]))
    def __str__(self):
...
class Edge:
    ...
    def __unicode__(self):
        return u"%s %s %s" % (self.node_from.data["name"], self.relation_symbol(), self.node_to.data["name"])
    def __str__(self):
        return self.__unicode__()
    def __repr__(self):
        return self.__unicode__()    
